...
There are tools to make living in the command line awesome, like [[https://github.com/ajeetdsouza/zoxide][zoxide]], [[https://github.com/tmux/tmux/wiki][tmux]] or [[https://zellij.dev/][zellij]], and [[https://github.com/nvbn/thefuck][The Fuck]]. The stock shell can be miserable, but it becomes so awesome as soon as you get used to it. A good terminal also makes a difference. [[https://ghostty.org/][Ghostty]] is awesome.
...
I feel the need to rant about Linux. Gimmie a minute. Skip if you're lame.

*** Boomer Rant

I went through all of the stages of operating systems. Grew up with macOS (my family is entirely macOS and I use a base M1 Macbook Air from time to time, so I am still familiar with their stuff), built my PC and used Windows 10 at the beginning of Covid, started developing and used WSL, and ditched Windows 11 for Linux. I've gone through Ubuntu, Mint, Fedora, Arch, a weird Nix phase that lasted about a week across a variety of machines from the worst laptops to some crazy high end PCs, and now Omarchy with the CachyOS kernel (which, aside from the installer, is just Arch under the hood).

I've tried all of the major desktop enviornments (except for Niri, but Hyprland has native scrolling now so I feel no pressure to). Omarchy is, in my opinion, the best, especially with Hyprland supporting scrolling. I've been using it for almost a whole year (since Omarchy 3 released), only to deviate for a one week Nix bender. Once you get used to Omarchy, it's impossible to go back to anything else.

At the very least, being familiar with Linux, especially in the world of tech, is important. The world runs on Linux.
...
** Boomer Rant Part Two: The Wonders of Org Mode

[[https://orgmode.org/][Org mode]] is [[https://www.youtube.com/watch?v=XRpHIa-2XCE][not just a markup language]]. It is capable of so much more than what I'm using it for. To see the results, open this in a text editor, or in the code inspector of GitHub.
...
Boomer rant part two over.

** Alla fine

Ultimately, this is no longer my project. It's in your hands now.

Take care of her.

Goodbye, darling. I'll miss you very much.
...
I made a fancy diagram for my AGU25 presentation. I want to show it off. It works in dark mode, the only true way to have your computer. I seared my retinas confirming this does not work in light mode. Be thankful for the sacrifices I make.
...
#+begin_quote
[!NOTE]
This will make the virtual enviornment folder as ~venv~ by default. If you care enough to use ~.venv~, you're smart enough to know what to modify.
#+end_quote
...
# The latest version constantly updates, so use the latest command from their website
# For me, I use the following since I have an AMD graphics card with ROCm:
pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/rocm6.3
#+end_src

PyTorch on macOS already comes with ~mps~, so no need to install anything fancy. Just remember to pass ~--device mps~ to take advantage of ~mps~.
...
Usually copying a file (~C-c~) and pasting it (~C-V~, control + shift + v) into the terminal also works. macOS's ~S-v~ (command/super + v) works with most terminals (in macOS) just fine, but most Linux terminals require ~C-V~  to paste into terminals. Omarchy (btw) supports ~S-v~ in terminals.

** Where can I get some models?

There are models available to download under the releases tab. Download a model (~.pth~ file) and use its path when prompted (or pass it in as a parameter with a flag). The latest release is usually the safest bet. I do not work on maintaining backwards compatibility, because Past Me made some really poor decisions.

#+begin_quote
[!CAUTION]
*Beware that larger models are quite large and VRAM intensive.* Use at your own risk. If your computer shuts down, it is probably because your VRAM got too hot and your computer went into a safety mode. If this occurs, try underclocking your GPU (specifically its VRAM) to limit temperatures, or pre-emptively setting your GPU fans to 100% (I have to do this with my dying RX 6800 XT).
#+end_quote

** How do I install some images/videos for testing?

A release containing two ~.avi~ videos and their corresponding folders of ~.png~ images is available in the releases tab under the ~data~ tag.

...
1. ~python3 -m venv venv~ to make a virtual environment inside the project directory.
2. ~source venv/bin/activate~ to go into the virtual environment. Do this every time you want to run this project, if you don't install the required packages systemwide (I do not recommend this, as I am a big fan of virtual enviornments and reproducibility).
3. ~pip install -r requirements.txt~ to install the requirements in ~requirements.txt~. If you have issues with torch, remove the venv (~rm -rf venv~), make a new virtual environment, install torch and torchvision /first/, and then install the requirements.
4. Have fun!

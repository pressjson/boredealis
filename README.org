...
@TODO:
1. Upload my models to the releases tab
2. Test /everything/
3. Update run.sh (or just make it obsolete)
   - Make it a bootstrap.sh instead
4. Allow main.py to both filter and smooth a video
   - Probably using a temp file to make life easier
5. A TUI
...
3. ~pip install -r requirements.txt~ to install the requirements listed in ~requirements.txt~.
   - If you have issues with torch, remove the venv (~rm -rf venv~), make a new virtual environment, install PyTorch /first/ (follow the command for your system on [[https://pytorch.org/get-started/locally/][the Pytorch website]]), and then install the requirements.

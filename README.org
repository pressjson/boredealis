...
I have this up top because I'm a lame bozo who wants to have this important section up top. Eventually it will go down below.

I am now trying a variety of different architectures instead of just the U-Net. You can find the complete list in [[file:models/][the models section]]. But for now, here are the rankings for each model I've tested and trained.

** The Actually Good Models

1. U-Net (~DeepUNet~). The OG King.
2. U-Net Plus Plus (~unetpp~). The OG King++, but sometimes C is better than C++.

** The Abominations

1. EDSR. It does not like this pipeline.
...
#+BEGIN_QUOTE
IMPORTANT: As far as I know, paths with spaces are NOT working. For example, this will /not/ work:

#+BEGIN_SRC bash
python3 main.py --input /home/pressjson/My Documents/boredealis/input_video.mov --output /home/pressjson/My Documents/boredealis/output_video.mov --model /home/pressjson/My Documents/boredealis/model.pth
#+END_SRC
...
Usually copying a file (~C-c~) and pasting it (~C-V~) into the terminal also works. macOS's ~C-v~ works with most terminals (in macOS) just fine, but most Linux terminals require ~C-V~ (control + shift + v) to paste into terminals. Omarchy (btw) supports ~S-v~.

...
def decompose_video_to_list(video_path, output_stem="output", verbose=False):
    """Turns a video into individual images in output_dir
    ...
    Returns:
        int, list: frames per second of the input video; list pair of (Image.Image, name_%04d.png)
        -1 on fail
    """
    if not os.path.exists(video_path):
        print(f"Error: video path does not exist: {video_path}")
    ...
    while True:
        ...
        images.append((image_pil, output_filename))

        frame_count += 1


    if verbose:
        print(f"  FPS returning: {fps}")
        print(f"  Image list type returning: {images}")
...
def compose_video_with_ram(images, output_name, fps=25, codec="mp4v", verbose=True):
    """Composes a video using OpenCV, using a list of Images.

    Args:
        images (list): List of image/name pairs (Image.Image, label_%04d.png).
        output_name (str): Path of the video to be saved to, extension and all
        fps (int): Framerate of the video to to be saved. By default it is 25
        codec (str): Codec to be used by OpenCV, by default avc1
        verbose (Bool): Lord help you, it prints too much info.
    ...
    images.sort(key=lambda f: int(re.match(r"(\w+)_(\d+)\.png", f[1]).group(2)))
